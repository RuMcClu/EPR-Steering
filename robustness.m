%this code calculates the EPR steering for a set of two qubit density matrices
rhoini=load('filename.mat');
%input should be table of 4x4 tables in .mat
...
%evolved states (sub-normalised)
    ...
    cvx_begin quiet sdp
   %code requires cvx package to work for convex optimisation
    cvx_precision best
    ...
    minimize(trace(Sigma1+Sigma2+Sigma3+Sigma4+Sigma5+Sigma6+Sigma7+Sigma8));
    for k=1:6
        ((Dl(1,k)*Sigma1+Dl(2,k)*Sigma2+Dl(3,k)*Sigma3+Dl(4,k)*Sigma4+Dl(5,k)*Sigma5+Dl(6,k)*Sigma6+Dl(7,k)*Sigma7+Dl(8,k)*Sigma8)-RHO(:,:,k))==semidefinite(2);
        %==semidefinite(2);
    end
    %for k=1:8
    %(SigmaL(:,:,k))>=0;
    ...
    answer(nnn)=trace(Sigma1+Sigma2+Sigma3+Sigma4+Sigma5+Sigma6+Sigma7+Sigma8)-1;
end
%export()
disp('finished')
