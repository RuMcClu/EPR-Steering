rhoini=load('OptProjDM2.mat');
projers=load('OptProjTest.mat');
rhofield=fieldnames(rhoini);
projfield=fieldnames(projers);
...
for nnn=1:length(rhofield),
    clearvars x y z XX Xnoneed YY Ynoneed ZZ Znoneed EigVec EigVal Lambda Dl PROJ RHO;
    RHO=zeros(2,2,S);
    Eig=zeros(2,2,S);
    PROJ=zeros(2,2,S);
    Dl=zeros(8,S);
    ...
    x=projers.(projfield{3*nnn-2});
    y=projers.(projfield{3*nnn-1});
    z=projers.(projfield{3*nnn});
    ...
    for i=1:2,
        for j=1:2,
            for k=1:2,
                ...
                Lambda(MM,3)=round(EigVec(:,:,k+4)'*z*EigVec(:,:,k+4));
